...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($email) && $_POST["usuario"] == "corr") {
        $emailErr = "No puede estar vacio.";
        $error = true;
    } elseif (!filter_var($email, FILTER_VALIDATE_EMAIL) && $_POST["usuario"] == "corr") {
    ...
    if (!$error) {
        if (!existeNombre($nUsuario) && $_POST["usuario"] == "login") {
            $nUsuarioErr = "El nombre de usuario no existe.";
            $error = true;
        } elseif (!verificarPassName($nUsuario, $password) && $_POST["usuario"] == "login") {
            $passwordErr = "El nombre o la contraseña es incorrecto.";
            $error = true;
        }
        
        if (!existeMail($email) && $_POST["usuario"] == "corr") {
            $emailErr = "El correo electrónico no existe.";
            $error = true;
        } elseif (!verificarPassEmail($email, $password) && $_POST["usuario"] == "corr") {
            $passwordErr = "El correo electrónico o la contraseña son incorrectos.";
            $error = true;
        }
    ...
    <div class="form-container">
        <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post">
            ...
            <div class="mb-3">
                ...
                <div class="form-check col-6">
                    <input type="radio" class="form-check-input" name="usuario" value="corr" id="corr">
